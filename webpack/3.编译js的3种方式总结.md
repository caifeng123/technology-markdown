...
当前市面上有3中方式，大家都各执一词踩一捧一。眼见为实耳听为虚，今天我们就来实践一下~
...
- 虽然对于webpack来说，是喷babel的。因为毕竟转译会让代码变得慢，且体积增加。但目前由于babel的生态好、插件多还是我们转译js jsx ts tsx的首选~
...
>
> 这是antd基于esm模块tree shaking制作的。那这个插件能帮我们做什么呢？来看看两个例子
...
> webpack官方推荐使用方法【但若需要转译兼容低版本还是用babel】，此种方法效率高速度快
...
| loader       | 结果                                                         | 结论                                                         |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Ts-loader    | ![img](https://raw.githubusercontent.com/caifeng123/pictures/master/image-20220501100119311.png) | 使用tsc进行类型检验 同时还会正常解析ts打包出js结果           |
...
> 相信大家会看到网络上各种esbuild的吹捧，就连vite也是基于esbuild，说实话webpack在这两者面前完全没有优势！
>
> 但由于本章的主角是webpack5因此，不能搬起石头砸自己的脚不是，因此这边将会介绍使用折中的 esbuild-loader。
>
> 说实话 个人觉得vite必将取代webpack，期待后期的vite教学~
